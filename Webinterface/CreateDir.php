<?php
		...
		if(sizeof($files) !== 0){
			...
			for($x = 0; $x < sizeof($files); $x++){
				...
				if(sizeof($arraySplit) === 6){
					...
					linkDataWithWorkspace($arraySplit[0], $arraySplit[2], $arraySplit[4], $arrayMeasureType[0], $newPath);
	...
	function linkDataWithWorkspace($workspace, $subject, $session, $measureType, $path){
		...
		$sqlInsertIntoLinkTable = "INSERT INTO NoAngstV2.Measurement (WorkspaceID, SubjectID, SessionID, MeasureTypeID, Path) 						VALUES (".$resultWorkspace['WorkspaceID'].", '".$subject."', '".$session."', ".$resultMeasureType['MeasureTypeID'].", '".$path."')";
